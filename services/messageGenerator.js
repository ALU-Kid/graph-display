// services/messageGenerator.js - Node 10 Compatible Smart Message Generator (FIXED)

function SmartMessageGenerator(options) {
  options = options || {};
  this.options = {
...
SmartMessageGenerator.prototype.generateMessages = function(weatherData, energyData, count) {
  count = count || 5;
  var messages = [];
...
SmartMessageGenerator.prototype.analyzeContext = function(weatherData, energyData) {
  var context = {
...
SmartMessageGenerator.prototype.analyzeWeather = function(weatherData) {
...
SmartMessageGenerator.prototype.analyzeEnergy = function(energyData) {
...
SmartMessageGenerator.prototype.analyzeTime = function() {
...
SmartMessageGenerator.prototype.analyzeSeason = function() {
  var now = new Date();
...
SmartMessageGenerator.prototype.checkEnergyMilestones = function(kwh, sessions) {
  var milestones = [];
...
SmartMessageGenerator.prototype.generateSingleMessage = function(contexts) {
  var strategies = [];
...
SmartMessageGenerator.prototype.generateWeatherMessage = function(weather) {
...
SmartMessageGenerator.prototype.generateEnergyMessage = function(energy) {
...
SmartMessageGenerator.prototype.generateMilestoneMessage = function(energy) {
...
SmartMessageGenerator.prototype.generateTimeMessage = function(time) {
  var templates = this.templates.time[time.period];
  return this.randomSelect(templates);
};

SmartMessageGenerator.prototype.generateWeekendMessage = function() {
  var templates = this.templates.time.weekend;
  return this.randomSelect(templates);
};

SmartMessageGenerator.prototype.generateSeasonalMessage = function(season) {
  var templates = this.templates.seasonal[season];
  return this.randomSelect(templates);
};

SmartMessageGenerator.prototype.weightedRandomSelect = function(items) {
  var totalWeight = 0;
...
SmartMessageGenerator.prototype.randomSelect = function(array) {
  return array[Math.floor(Math.random() * array.length)];
};

SmartMessageGenerator.prototype.isRecentMessage = function(message) {
  return this.recentMessages.indexOf(message) !== -1;
};

SmartMessageGenerator.prototype.addToRecentMessages = function(message) {
...
SmartMessageGenerator.prototype.validateMessage = function(message) {
...
SmartMessageGenerator.prototype.getContextSummary = function(weatherData, energyData) {
  var contexts = this.analyzeContext(weatherData, energyData);
  var summary = [];
...
// Export the constructor function and validation
